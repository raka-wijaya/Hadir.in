  ...
  const fetchAttendance = useCallback(async () => {
    ...
    try {
      ...
      const absRes = await fetch(
        `/api/absensi?karyawan_os_id=${encodeURIComponent(targetUserId)}`,
        {
          cache: "no-store",
        }
      );
      ...
      if (absRes.ok) {
        ...
        if (absData.success && Array.isArray(absData.data)) {
          const myAttendances = absData.data.filter(
            (a: any) =>
              String(a.karyawan_os_id || a.karyawanOsId) === String(targetUserId)
          );
      ...
      console.error("Gagal memuat data presensi pegawai OS:", err);
      setUserAttendances([]);
    } finally {
      setIsLoading(false);
    }
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-foreground flex items-center gap-2">
                  ...
                  Pekerjaan / Tugas yang Dikerjakan
                </label>

                <textarea
                  value={tugasDikerjakan}
                  onChange={(e) => setTugasDikerjakan(e.target.value)}
                  placeholder="Jelaskan pekerjaan atau pelayanan yang sudah kamu kerjakan hari ini..."
                  rows={4}
                  className="w-full rounded-xl border border-border bg-input px-3.5 py-3 text-xs font-medium outline-none resize-none focus:ring-2 focus:ring-primary/30 focus:border-primary"
                />
              </div>
              ...
              <div className="rounded-xl bg-muted/50 border border-border p-3">
                <p className="text-[11px] text-muted-foreground leading-relaxed">
                  Data alasan pulang cepat dan pekerjaan yang dikerjakan akan
