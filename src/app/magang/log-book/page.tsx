...
export default function MagangLogBookPage() {
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute("download", `Logbook_Magang_${new Date().toISOString().slice(0, 10)}.csv`);
      ...
      <div className="space-y-6 print:hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border pb-5">
          <div>
            ...
            <p className="text-xs md:text-sm text-muted-foreground font-medium mt-1">
              Catat dan pantau seluruh kegiatan harian, tugas, dan capaian magang Anda secara transparan.
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
          <div className="bg-card border border-border rounded-2xl p-4 md:p-5 space-y-1.5 shadow-card hover:border-primary/40 transition-all">
            ...
            <p className="text-[11px] text-muted-foreground">Aktivitas tercatat</p>
          </div>

          <div className="bg-card border border-border rounded-2xl p-4 md:p-5 space-y-1.5 shadow-card hover:border-primary/40 transition-all">
            <div className="flex items-center justify-between text-muted-foreground">
              <span className="text-[11px] font-extrabold uppercase tracking-wider">Total Jam Kerja</span>
          ...
          <div className="bg-card border border-border rounded-2xl p-4 md:p-5 space-y-1.5 shadow-card hover:border-primary/40 transition-all">
            ...
            <p className="text-[11px] text-muted-foreground">Paling sering dikerjakan</p>
          </div>
        </div>
        ...
        <div className="bg-card border border-border rounded-2xl shadow-card overflow-hidden">
          <div className="p-4 border-b border-border flex items-center justify-between gap-4">
            <h2 className="text-sm md:text-base font-extrabold text-foreground flex items-center gap-2">
              <span>Daftar Riwayat Logbook</span>
          ...
          {isLoading ? (
            ...
            <div className="py-16 text-center space-y-3">
              ...
              <p className="text-xs text-muted-foreground max-w-sm mx-auto">
                Anda belum memiliki logbook yang sesuai dengan filter. Klik tombol &ldquo;Catat Logbook&rdquo; untuk mulai mencatat kegiatan.
              </p>
              <button
                onClick={() => {
                  resetForm();
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-xs flex items-center justify-center p-4 print:hidden">
          <div className="bg-card border border-border rounded-2xl max-w-4xl w-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden animate-in fade-in zoom-in-95">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {logbooks.length} aktivitas siap dicetak. Gunakan Save as PDF di jendela cetak browser.
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-4 border-b border-border bg-muted/10 grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
              ...
              <div>
                <label className="block font-bold text-foreground mb-1">NIP / Jabatan Pembimbing</label>
                <input
                  type="text"
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-6 bg-neutral-900/10 dark:bg-black/40">
              <div className="bg-white text-black p-8 rounded-lg shadow-md max-w-3xl mx-auto space-y-6 text-xs font-sans">
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 p-3 rounded-lg border border-neutral-300 bg-neutral-50 text-[11px]">
                  <div>
                    <span className="font-semibold text-neutral-600">Nama Mahasiswa / Siswa: </span>
                    <strong className="text-black">{user?.name || user?.nama || "—"}</strong>
                  </div>
                  <div>
                    <span className="font-semibold text-neutral-600">Instansi / Kampus / Sekolah: </span>
                    <strong className="text-black">{user?.institution || user?.sekolah_kampus || "—"}</strong>
                  </div>
                  <div>
                    <span className="font-semibold text-neutral-600">Program Studi / Jurusan: </span>
                    <strong className="text-black">{user?.study_program || user?.studyProgram || "—"}</strong>
                  </div>
                  <div>
                    <span className="font-semibold text-neutral-600">No. Identitas / NIM / NIS: </span>
                    <strong className="text-black">{user?.identity_number || user?.identityNumber || "—"}</strong>
                  </div>
                </div>
                    ...
                    {logbooks.map((item, idx) => {
                      ...
                      return (
                        <tr key={item.id} className="border-b border-neutral-400">
                          ...
                          <td className="border border-black p-1.5 text-center font-medium">{formatTanggalIndo(item.tanggal)}</td>
                          <td className="border border-black p-1.5 text-center font-mono">{formatWaktu(item.waktu_mulai)} - {formatWaktu(item.waktu_selesai)}</td>
                ...
                <div className="grid grid-cols-2 gap-8 pt-6 text-[11px] text-center">
                  <div className="space-y-16">
                    ...
                    <div>
                      <p className="font-bold underline uppercase">{user?.name || user?.nama || "( ........................................ )"}</p>
            ...
            <div className="p-4 border-t border-border bg-card flex items-center justify-between gap-3 shrink-0">
              <span className="text-xs text-muted-foreground">
                Tip: Pilih opsi <strong>Save as PDF</strong> pada jendela cetak browser untuk menyimpan sebagai PDF.
              </span>
              <div className="flex items-center gap-2">
                <button
                  type="button"
